...
int main(int argc, char **argv)
{
	...
	int ctrl_fd1, rpmsg_fd1;
	struct rpmsg_endpoint_info eptinfo1;
	struct pollfd fds1;
	char *buff1;
	data_packet test_data1;
	data_packet test_data_r1;
	char buff_r1[MAX_DATA_LENGTH];
	...
	test_data1.command = DEVICE_CORE_CHECK;
	buff1 = test_data1.data;
	...
	ctrl_fd1 = open("/dev/rpmsg_ctrl1", O_RDWR);
	if (ctrl_fd1 < 0) {
		perror("open rpmsg_ctrl1 failed.\n");
		return -1;
	}

	memcpy(eptinfo1.name, "yyyy", 32);
	eptinfo1.src = 0;
	eptinfo1.dst = 0;
	...
	rpmsg_fd1 = open("/dev/rpmsg1", O_RDWR);
	if (rpmsg_fd1 < 0) {
	...
	if (argc > 2 && strcmp(argv[1], "stop") == 0) {
		if (strcmp(argv[2], "0") == 0) {
			...
			printf("want to stop 1\n");
			test_data1.command = DEVICE_CORE_STOP;
			test_data1.length = 1;
			ret = write_full(rpmsg_fd1, &test_data1, sizeof(data_packet));
			if (ret < 0) {
				perror("write_full failed.\n");
			}
		} else {
	...
	while (count) {
		...
		leng = snprintf(buff1, MAX_DATA_LENGTH, "%s%d", "Hello World! No:", ++no1);
		test_data1.length = leng;
		...
		memset(&test_data_r1, 0, sizeof(data_packet));
		ret = read_full(rpmsg_fd1, &test_data_r1, sizeof(data_packet));
		if (ret < 0) {
			perror("read_full failed.\n");
			goto err3;
		}

		memcpy(buff_r1, test_data_r1.data, test_data_r1.length);
		/* output message */
		printf("rpmsg1 here received message: %s\n", buff_r1);

		usleep(5000);

		count--;
		memset(buff_r, 0, MAX_DATA_LENGTH);
		memset(buff_r1, 0, MAX_DATA_LENGTH);
	}
