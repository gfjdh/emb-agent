...
- 修订时间: 2026-4-28
- 项目负责人: 两组（边缘推理盒子组 + 嵌入式程序优化Agent组）

---

## 一、项目背景与目标

### 1.1 边缘推理盒子（边缘推理盒子组负责）
...
```
用户: "分析一下当前板子性能瓶颈"
  → SSH连接板子 → 采集性能指标 → 结合项目代码分析 → 生成优化建议
```
...
```
Agent组任务依赖:
  网络配置完成 → SSH连接验证 → 性能监控工具 → 瓶颈分析Agent

边缘推理盒子组任务依赖:
  网络配置完成 → 开发环境准备 → 模型部署 → API服务
